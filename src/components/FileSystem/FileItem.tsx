...
const FileItem: React.FC<FileItemProps> = ({
    ...
    const renderIcon = () => {
        if (item.is_folder) {
            return <Folder size={iconSize} style={{ color: folderColor }} />;
        }

        const IconComponent = fileTypeConfig?.icon;
        if (IconComponent) {
            return <IconComponent size={iconSize} style={{ color: fileTypeConfig.color }} />;
        }
    ...
    return (
        <div
            className="inline-flex flex-col items-center group cursor-pointer p-2 hover:bg-gray-500 rounded"
            style={{
